...
export function RequestMeetingButton({ ownerId, ownerName }: RequestMeetingButtonProps) {
  const { user } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Your meeting request has been sent to ${ownerName}`,
      })
  ...
  // Only employees can request meetings with owners
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Request Meeting with {ownerName}</DialogTitle>
            <DialogDescription>
              Fill out the form below to request a meeting. The owner will be notified and can approve, schedule, or
              deny your request.
            </DialogDescription>
          </DialogHeader>
