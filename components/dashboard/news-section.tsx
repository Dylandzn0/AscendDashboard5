...
export function UpcomingEvents() {
  ...
  useEffect(() => {
    const loadNewsItems = () => {
      ...
      const meetingRequests = loadData("meeting-requests", [])
      if (Array.isArray(meetingRequests)) {
        meetingRequests.forEach((request) => {
          items.push({
            id: `meeting-${request.id}`,
            type: "meeting",
            title: request.subject || "Meeting Request",
            description: request.message || "No details provided",
            date: request.requestedDateTime || new Date().toISOString(),
            status: request.status || "pending",
            link: "/meeting-requests",
            relatedTo: request.clientName,
          })
        })
      }
